...
const friends: FastifyPluginAsync = async (fastify, opts): Promise<void> => {
	fastify.post('/request', { preValidation: [fastify.authenticate] }, async (req: any, reply: any) => {
	...
	fastify.post('/accept', { preValidation: [app.authenticate] }, async (req: any, reply: any) => {
	...
	fastify.post('/decline', { preValidation: [fastify.authenticate] }, async (req: any, reply: any) => {
		const { requestId } = req.body;
		const request = await getFriendRequestById(requestId);
		if (!request) {
			return reply.code(404).send({ message: 'Request not found' });
	...
	fastify.post('/remove', { preValidation: [fastify.authenticate] }, async (req: any, reply: any) => {
