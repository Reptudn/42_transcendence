...
const default_titles_first =
  require("../../../data/defaultTitles.json").default_titles_first;
const default_titles_second =
  require("../../../data/defaultTitles.json").default_titles_second;
const default_titles_third =
  require("../../../data/defaultTitles.json").default_titles_third;
...
export async function registerGoogleUser(googleUser: GoogleUserInfo) {
  ...
  console.log("trying to register google user");
  ...
  console.log("Adding google user to db with id:", googleUser.id);
  await db.run(
...
export async function incrementUserClickCount(
  ...
  await db.run("UPDATE users SET click_count = click_count + ? WHERE id = ?", [
    increment,
    userId,
  ]);
  const user = await db.get(
